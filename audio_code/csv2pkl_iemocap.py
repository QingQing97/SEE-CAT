...
# ==================== 路径配置 ====================
BASE_DIR = "audio_video_code/audio_code/audio2feature/IEMOCAP_Splits_1011"
...
def get_gpu_usage_mb():
    ...
    for i in range(device_count):
        ...
        gpu_info.append((i, mem_info.used / (1024 * 1024), mem_info.total / (1024 * 1024)))
    return gpu_info
...
def main():
    ...
    if not os.path.exists(BASE_DIR):
        logger.error(f"CSV 目录不存在: {BASE_DIR}")
        return
    ...
    logger.info(f"共找到 {len(csv_files)} 个 CSV 文件")
